...
StubEmailProvider logs instead of sending (dev default). SesEmailProvider
sends real email via Amazon SES once EMAIL_DELIVERY_MODE is set to "ses" and
SES_FROM_EMAIL is configured. get_email_provider() selects between them.
EMAIL_DELIVERY_MODE is independent of OTP_DELIVERY_MODE (the phone/SMS
channel's own setting) -- see otp.py's _delivery_mode() helper.
"""
...
class EmailSendError(RuntimeError):
    ...
    caller instead of letting the underlying provider exception (a boto3
    error) escape uncaught -- an uncaught exception here crashes past
...
class SesEmailProvider:
    ...
    -- SES rejects the send otherwise, at the account level, not something
    this class validates itself. Uses the ECS task's
