...
import { getAuthStage, useMobileJourney } from "./mobileJourneyContext";
...
/**
 * 4 Chronological Milestones along the Organic Journey:
 * Milestone 1 (Email) -> Milestone 2 (Email OTP) -> Milestone 3 (Phone) -> Milestone 4 (Phone OTP)
 * Extended coordinates to span the full width naturally.
 */
const AUTH_MILESTONES = [
...
// Expressive organic curve traversing the 4 milestones and naturally continuing to the absolute left and right edges
const ROUTE_PATH_D = "M 0 41 C 12 41, 24 39, 36 38 C 70 26, 106 21, 136 22 C 168 23, 204 39, 238 38 C 270 37, 306 23, 340 22 C 352 22, 364 23, 375 25";
 ...
 * - 4 narrative authentication stage illustrations.
 */
export function AuthRoadmapSvg({
  ...
  const stageInfo = getAuthStage(rawActiveStep, explicitStepIndex);
  const shouldReduceMotion = useReducedMotion() || isTestEnv;

  const currentMilestone = AUTH_MILESTONES[stageInfo.stepIndex] ?? AUTH_MILESTONES[0];
  const ringSize = 17;
  ...
  return (
    ...
    >
      ...
      {/* 4 Connected Authentication Journey Stages */}
      ...
      {/* Upcoming Inactive Route Guide: Subtle light-green dotted curved path matching mobile */}
      <path
        d={ROUTE_PATH_D}
        stroke="#22C55E"
      ...
      {/* Radiant Green Path Under-Glow (Alive glowing trace) */}
      <motion.path
        d={ROUTE_PATH_D}
        stroke="#22C55E"
        ...
        strokeLinejoin="round"
        fill="none"
        initial={shouldReduceMotion ? { pathLength: stageInfo.progress } : { pathLength: 0.10 }}
      ...
      <motion.g
        initial={
          shouldReduceMotion
            ...
            : { x: AUTH_MILESTONES[0].x, y: AUTH_MILESTONES[0].y }
        }
        animate={{
