...
function formatStatValue(
  ...
  const formatted = new Intl.NumberFormat("en-IN", formatOptions).format(
    value
  );
...
export function ChartStatFlow({
  ...
  return (
    <>
      ...
      <span className={cn("text-[var(--color-ink)] tabular-nums type-data-large", valueClassName)}>
        {numberFlowReady ? (
          <NumberFlow
      ...
      <span className={cn("mt-0.5 text-[var(--color-text-secondary)] type-caption", labelClassName)}>
        {label}
      </span>
    </>
  );
