...
| AMFI TER page | Scheme-wise TER | Monthly (per AMFI's disclosure cadence) | **Resolved approach**: same reverse-engineering method already proven against AMFI's own site family (multiple existing open-source scrapers do exactly this for AMFI's NAV and ARN-holder data) — capture the underlying JSON endpoint the page's filter UI calls via browser network inspection, call it directly; fall back to headless-browser automation of the filter UI if no stable endpoint is found. Remaining work is a short implementation task, not an open research question. | `scheme_ter` (Postgres) |
| AMFI AAUM page | Scheme-wise AAUM | Quarterly | Same resolved approach as TER — same site family, same method | `scheme_aaum` (Postgres) |
| AMFI ARN lookup | Distributor name/status per ARN | On-demand (first time an ARN is seen), not scheduled | Same resolved approach — and directly precedented: an existing open-source scraper (`amfi-arn-data-scrapper`) already does this against AMFI's ARN-holder data specifically | `arn_directory` (Postgres) |
| NSE Indices (niftyindices.com) | Nifty 50/500/LargeMidcap250/Midcap150 historical levels | Daily | **Resolved, concretely**: niftyindices.com's own backend endpoint (`Backpage.aspx/getHistoricaldatatabletoString`, session-cookie-based POST, JSON response) is documented and already used by a maintained open-source library — call it directly rather than parsing rendered HTML | `benchmark_index_history` (Postgres) |
