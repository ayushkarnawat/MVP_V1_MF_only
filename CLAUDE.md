...
- **Refined Hand-Drawn Pen Stroke Underline (`HandDrawnUnderline.tsx`, `Landing.tsx`)**: Built an expressive, organic SVG ink stroke (`strokeWidth="2.2"`, `viewBox="0 0 120 14"`) under "Log in" / "Sign up" links on web and mobile auth pages. Features natural asymmetric pen-stroke bends in Unifolio green (`text-[var(--color-accent)]`) that are clearly visible at normal viewing sizes on mobile and desktop.
